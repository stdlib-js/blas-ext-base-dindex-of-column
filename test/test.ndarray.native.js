...
var dindexOfColumn = tryRequire( resolve( __dirname, './../lib/ndarray.native.js' ) );
var opts = {
...
tape( 'the function has an arity of 12', opts, function test( t ) {
	t.strictEqual( dindexOfColumn.length, 12, 'returns expected value' );
	t.end();
});
...
tape( 'the function returns an invalid index when M is less than or equal to zero (row-major)', opts, function test( t ) {
	...
	data = ROW_MAJOR_DATA;
	out = dindexOfColumn( 0, data.N, new Float64Array( data.A ), data.strideA1, data.strideA2, data.offsetA, new Float64Array( data.x ), data.strideX, data.offsetX, new Uint8Array( data.N ), 1, 0 );
...
tape( 'the function returns an invalid index when N is less than or equal to zero (row-major)', opts, function test( t ) {
	...
	data = ROW_MAJOR_DATA;
	out = dindexOfColumn( data.M, 0, new Float64Array( data.A ), data.strideA1, data.strideA2, data.offsetA, new Float64Array( data.x ), data.strideX, data.offsetX, new Uint8Array( data.N ), 1, 0 );
...
tape( 'the function returns the index of the first column matching a search vector (row-major)', opts, function test( t ) {
	...
	data = ROW_MAJOR_DATA;
	out = dindexOfColumn( data.M, data.N, new Float64Array( data.A ), data.strideA1, data.strideA2, data.offsetA, new Float64Array( data.x ), data.strideX, data.offsetX, new Uint8Array( data.N ), 1, 0 );
